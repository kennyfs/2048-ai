...
class Trainer:
    ...
    def __init__(
        self, initial_checkpoint, model: model.Network, config: my_config.Config
    ):
        self.config = config
        self.model = model
        """
    ...
    def run_update_weights(
    ...
    ):  # max_steps is for observing loss
        ...
        while (
            self.training_step / max(1, shared_storage.get_info("num_played_steps"))
            ...
            print(
                f"training_step:{self.training_step},lr={self.optimizer.learning_rate}"
            )
            st = time.time()
            (
                priorities,
            ...
            shared_storage.append_loss(
                total_loss, value_loss, reward_loss, policy_loss, l2_loss
            )
            shared_storage.append_output(
                value_initial,
                value_recurrent,
                reward,
                value_initial_delta,
            ...
            print(
                f'ratio:{self.training_step / max(1, shared_storage.get_info("num_played_steps"))}'
            )
    ...
    def update_weights(self, batch, shared_storage):
        ...
        if self.config.support:
            target_value_batch = model.scalar_to_support(
                target_value_batch, self.config.support
            )
            target_reward_batch = model.scalar_to_support(
                target_reward_batch, self.config.support
            )
        else:
        ...
        assert (
            list(observation_batch.shape)
            == [self.batch_size] + self.config.observation_shape
            and list(action_batch.shape) == [self.batch_size, self.num_unroll_steps]
            and list(target_value_batch.shape)
            == [self.batch_size, self.num_unroll_steps + 1, 2 * self.config.support + 1]
            and list(target_reward_batch.shape)
            == [self.batch_size, self.num_unroll_steps + 1, 2 * self.config.support + 1]
            and list(target_policy_batch.shape)
            == [self.batch_size, self.num_unroll_steps + 1, 4]
            and (not self.config.PER or list(weight_batch.shape) == [self.batch_size])
        ), f"batch shape error,{observation_batch.shape},{action_batch.shape},{target_value_batch.shape},{target_reward_batch.shape},{target_policy_batch.shape},{weight_batch.shape}"
        ...
        def loss_fn():
            ...
            ):  ### start to check data processing here
            ...
            assert (
                len(predictions) == self.num_unroll_steps + 1
            ...
            total_value_loss, total_reward_loss, total_policy_loss = (
            ...
            for i, prediction in enumerate(predictions):
                ...
                value_loss, reward_loss, policy_loss = self.loss_function(
                    value,
                    reward,
                    policy_logits,
                    target_value,
                    target_reward,
                    target_policy,
                    self.config.support != 0,
                )
                if i == 0:
                    total_value_loss += value_loss
                    total_policy_loss += policy_loss
                else:
                    total_value_loss += scale_gradient(
                        value_loss, 1.0 / (self.num_unroll_steps - 1)
                    )
                    total_reward_loss += scale_gradient(
                        reward_loss, 1.0 / (self.num_unroll_steps - 1)
                    )
                    total_policy_loss += scale_gradient(
                        policy_loss, 1.0 / (self.num_unroll_steps - 1)
                    )
                if self.config.support:
                    pred_value_scalar = model.support_to_scalar(
                        value, self.config.support, True
                    )
                else:
                ...
                if step.value == 0:
                    if i == 0:
                        ...
                        last_value_initial_delta.set(
                            pred_value_scalar[index] - target_value_scalar[index][i]
                        )
                        last_value_initial.set(pred_value_scalar[index])
                    elif i == 1:
                        index = np.random.choice(len(pred_value_scalar))
                        last_value_recurrent_delta.set(
                            pred_value_scalar[index] - target_value_scalar[index][i]
                        )
                        last_value_recurrent.set(pred_value_scalar[index])
                        index = np.random.choice(len(pred_value_scalar))
                        reward = np.expand_dims(reward[index], 0)
                        last_reward_delta.set(
                            model.support_to_scalar(reward, self.config.support, True)[
                                0
                            ].numpy()
                            - target_reward_scalar[index][i]
                        )
                        last_reward.set(
                            model.support_to_scalar(reward, self.config.support, True)[
                                0
            ...
            loss = (
                total_value_loss * self.config.loss_weights[0]
                + total_reward_loss * self.config.loss_weights[1]
                + total_policy_loss * self.config.loss_weights[2]
            ) / sum(self.config.loss_weights)
            """if self.config.PER:
