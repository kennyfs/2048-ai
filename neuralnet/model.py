...
def scale_hidden_state(to_scale_hidden_state: np.array):
    ...
    if hidden_state_as_matrix:
        to_scale_hidden_state = tf.reshape(
            to_scale_hidden_state, (shape[0], -1)
        )  # flatten from [batch_size,hidden_state_size_x,hidden_state_size_y] to [batch_size,hidden_state_size_x*hidden_state_size_y]
    ...
    scale_encoded_state = tf.where(
        scale_encoded_state < 1e-5, scale_encoded_state + 1e-5, scale_encoded_state
    )  # avoid divided by 0 or too small value
    encoded_state_normalized = (
        to_scale_hidden_state - min_encoded_state
    ) / scale_encoded_state
    if hidden_state_as_matrix:
        encoded_state_normalized = tf.reshape(
            encoded_state_normalized, shape
        )  # and reshape to original shape
    return encoded_state_normalized


def support_to_scalar(
    logits, support_size, from_logits=True
):  # logits is in shape (batch_size,full_support_size)
    """
...
def scalar_to_support(x, support_size):
    ...
    logits = tf.zeros(
        (length * (2 * support_size + 1))
    )  # flattened of (length , 2 * support_size + 1)
    ...
    logits = tf.tensor_scatter_nd_update(logits, indices=indices, updates=1 - prob)
    ori_indices = ori_indices + 1
    prob = tf.where(2 * support_size < ori_indices, 0.0, prob)
    ori_indices = tf.where(2 * support_size < ori_indices, 0, ori_indices)
    indices = ori_indices + indices_to_add
    indices = tf.expand_dims(indices, axis=-1)  # index is in 1-dimensional
    logits = tf.tensor_scatter_nd_update(logits, indices=indices, updates=prob)
...
def batch_action_to_onehot(action_batch, board_size, matrix=False):
    ...
    if matrix:
        ret = np.concatenate(
            [
                np.expand_dims(
                    action_to_onehot(action, board_size, matrix=True), axis=0
                )
...
NetworkOutput = collections.namedtuple(
    "NetworkOutput", ["reward", "hidden_state", "value", "policy"]
)
...
class AbstractNetwork(ABC):
    ...
    def initial_inference(self, observation) -> NetworkOutput:
        ...
        return NetworkOutput(
            policy=policy, value=value, reward=None, hidden_state=hidden_state
        )
    ...
    def recurrent_inference(self, hidden_state, action, matrix=False) -> NetworkOutput:
        ...
        if not matrix and len(hidden_state.shape) == 4:
            matrix = True
        action_onehot = batch_action_to_onehot(
            action, self.config.board_size, matrix=matrix
        )
        hidden_state, reward = self.dynamics(hidden_state, action_onehot)
        hidden_state = scale_hidden_state(hidden_state)
        policy, value = self.prediction(hidden_state)
        return NetworkOutput(
            policy=policy, value=value, reward=reward, hidden_state=hidden_state
        )
...
class FullyConnectedNetwork(AbstractNetwork):
    def __init__(self, config):
        ...
        self.trainable_variables = (
            self.representation_model.trainable_variables
            + self.dynamics_model.trainable_variables
            + self.prediction_model.trainable_variables
        )
    ...
    def get_weights(self):
        return {
            "representation": self.representation_model.get_weights(),
            "dynamics": self.dynamics_model.get_weights(),
            "prediction": self.prediction_model.get_weights(),
        }
...
class dynamics(tf.keras.Model):
    def __init__(
        ...
        self.reward_output = [
            tf.keras.layers.Dense(size, activation="relu") for size in reward_layers
        ] + [tf.keras.layers.Dense(support * 2 + 1)]
        self.build([1] + input_shape)
...
class prediction(tf.keras.Model):
    def __init__(
        ...
        self.flatten = tf.keras.layers.Flatten()
        self.dense_value = [
            tf.keras.layers.Dense(size, activation="relu") for size in value_layers
        ] + [tf.keras.layers.Dense(support * 2 + 1)]
        self.dense_policy = [
            tf.keras.layers.Dense(size, activation="relu") for size in policy_layers
...
class ResNetNetwork(AbstractNetwork):
    def __init__(self, config: my_config.Config):
        ...
        self.representation_model = representation(
            config.observation_shape, config.num_channels, config.num_blocks - 1
        )
...
class Manager:
    ...
    async def push_queue(
        self, features: np.array, network: str
    ):  # network means which to use. If passing string consumes too much time, pass int instead.
        if self.queue:
        ...
        while margin > 0:
            if self.are_all_queues_empty():
                ...
                if self.are_all_queues_empty():
                    ...
                    await asyncio.sleep(1e-3)
                continue
            for name, queue, func in zip(
                ("representation", "dynamics", "prediction"),
                (self.representation_queue, self.dynamics_queue, self.prediction_queue),
                (self.representation, self.dynamics, self.prediction),
            ):
                if queue.empty():
                    continue
                item_list = [
                ...
                inputs = np.concatenate(
                    [np.expand_dims(item.inputs, axis=0) for item in item_list], axis=0
                )
                ...
                if name == "representation":
                    ...
                    assert hidden_state.shape[0] == len(item_list), (
                        "sizes of hidden_state("
                        + hidden_state.shape
                        + ") and item_list("
                        + len(item_list)
                        + ") don't match, this should never happen."
                    )
                    for i in range(len(item_list)):
                        item_list[i].future.set_result(hidden_state[i])
                elif name == "dynamics":
                    hidden_state, reward = results
                    assert hidden_state.shape[0] == len(item_list) and reward.shape[
                        0
                    ] == len(item_list), (
                        "sizes of hidden_state("
                        + hidden_state.shape
                    ...
                    assert policy.shape[0] == len(item_list) and value.shape[0] == len(
                        item_list
                    ), (
                        "sizes of policy("
                        + policy.shape
...
class Predictor:
    ...
    async def initial_inference(self, observation) -> NetworkOutput:
        ...
        """
        hidden_state = await self.get_outputs(
            observation, "representation"
        )  # already scaled
        policy, value = await self.get_outputs(hidden_state, "prediction")
        print(f"value:{value}")
        return NetworkOutput(
            reward=0, hidden_state=hidden_state, value=value, policy=policy
        )

    async def recurrent_inference(
        self, hidden_state: np.array, action: np.array
    ) -> NetworkOutput:
        """
        ...
        if type(action) in (list, np.array):
            action = action[0]
        action = action_to_onehot(
            action, self.config.board_size, matrix=(self.mode == "resnet")
        )
        inputs = np.concatenate((hidden_state, action), axis=0)
        new_hidden_state, reward = await self.get_outputs(
            inputs, "dynamics"
        )  # hidden state is already scaled

        policy, value = await self.get_outputs(new_hidden_state, "prediction")

        return NetworkOutput(
            reward=reward, hidden_state=new_hidden_state, value=value, policy=policy
        )
