...
class ReplayBuffer:
    ...
    def save_game(self, game_history, save_to_file=True):
        ...
        if self.config.save_game_to_file and save_to_file:
            game_history.save(
                os.path.join(
                    self.config.save_game_dir, f"{self.num_played_games}.record"
                )
            )
    ...
    def load_games(self, first_game_id, last_game_id):
        for i in range(first_game_id, last_game_id + 1):
            ...
            game_history.load(
                os.path.join(self.config.load_game_dir, f"{i}.record"), self.config
            )
        ...
        for game_id, game_history, game_prob in self.sample_n_games(batch_size):
            ...
            if self.rotate:
                ...
                for _ in range(rotate):
                    ...
                    policies = [
                        [policy[3], policy[2], policy[0], policy[1]]
            ...
            if self.flip and random.randint(0, 1) == 1:
                ...
                policies = [
                    [policy[1], policy[0], policy[2], policy[3]] for policy in policies
                ]
    ...
    def compute_target_value(self, game_history: selfplay.GameHistory, index):
        ...
        for i, reward in enumerate(
            game_history.reward_history[
                index + 1 : min(game_history.length, bootstrap_index + 1)
            ]
        ):
    ...
    def make_target(self, game_history, state_index):
        ...
        for current_index in range(
            state_index + 1,
            state_index + self.config.num_unroll_steps + 1,  # +1 to +num_unroll_steps
        ):
...
class Reanalyze:
    ...
    def reanalyze(self, replay_buffer, shared_storage):
        for game_id, game_history in replay_buffer.buffer.items():
            ...
            shared_storage.set_info("num_reanalyzed_games", self.num_reanalyzed_games)
